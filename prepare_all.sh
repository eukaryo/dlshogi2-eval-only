...
UPSTREAM_OWNER="${UPSTREAM_OWNER:-TadaoYamaoka}"
UPSTREAM_REPO="${UPSTREAM_REPO:-python-dlshogi2}"
# Default to the latest commit currently visible on main at the time this repo
# skeleton was prepared. Override this before a frozen public release if you want
# a different exact upstream snapshot.
UPSTREAM_COMMIT="${UPSTREAM_COMMIT:-358a704}"
...
UPSTREAM_ARCHIVE_PATH="${UPSTREAM_ARCHIVE_PATH:-$THIRD_PARTY_UPSTREAM_DIR/${UPSTREAM_REPO}-${UPSTREAM_COMMIT}.tar.gz}"
UPSTREAM_SNAPSHOT_DIR="${UPSTREAM_SNAPSHOT_DIR:-$THIRD_PARTY_UPSTREAM_DIR/${UPSTREAM_REPO}-${UPSTREAM_COMMIT}}"
CHECKPOINT_PATH="${CHECKPOINT_PATH:-$UPSTREAM_SNAPSHOT_DIR/checkpoints/${CHECKPOINT_NAME}}"
...
EOF_POS
  ...
  "$VENV_PY" - <<'PY' "$CHECKPOINT_PATH" "$ARTIFACT_DIR/reference_startpos.pt2" "$EXPORT_POSITION"
import sys
import torch
