...
WEIGHTS_MANIFEST_TYPE = "openheart_weights_manifest_v1"
CONTRACT_VERSION = 1
DEFAULT_PRODUCER_NAME = "dlshogi2-eval-only"
DEFAULT_PACKAGE_ID_PREFIX = "openheart-dlshogi2"
...
@dataclass(frozen=True)
class OpenHeartPackageArtifacts:
    package_dir: str
    manifest_path: str
    package_id: str
    graph_path: str
...
def write_safetensors_weights(
    ...
    if save_safetensors_file is None:
        raise ImportError(
            "safetensors is required for openheart package export. "
...
def build_openheart_package_id(
...
def validate_openheart_package_dir(package_dir: str | Path) -> list[str]:
    errors: list[str] = []
...
def export_openheart_package(
    ...
    *,
    ...
    package_id = build_openheart_package_id(manifest_payload, prefix=package_id_prefix)
    ...
    if errors:
        raise RuntimeError("exported openheart package failed validation:\n- " + "\n- ".join(errors))

    return OpenHeartPackageArtifacts(
