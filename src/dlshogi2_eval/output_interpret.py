...
def _load_outputs_from_dir(path: str | Path) -> dict[str, np.ndarray]:
    ...
    raise FileNotFoundError(
        "could not find external outputs under directory. Expected one of: "
...
def interpret_external_outputs(
    *,
    ...
    backend = BackendSpec(kind="external", device="external", precision="external", use_autocast=False)
