...
from dlshogi2_eval.openheart_package import (
...
def test_build_openheart_package_id_is_stable_for_same_payload() -> None:
    payload = {
        "package_type": "openheart_model_package_v1",
        "contract_version": 1,
    ...
    assert build_openheart_package_id(payload) == build_openheart_package_id(dict(payload))
...
def _write_minimal_valid_package(tmp_path: Path) -> Path:
    ...
    weights_manifest = {
        "package_type": "openheart_weights_manifest_v1",
        "weight_entries": [
            {
                "name": "linear.weight",
                "dtype": "float32",
    ...
    manifest = {
        "package_type": "openheart_model_package_v1",
        "contract_version": 1,
        "package_id": "openheart-dlshogi2-test",
...
def test_validate_openheart_package_dir_accepts_minimal_valid_package(tmp_path: Path) -> None:
    package_dir = _write_minimal_valid_package(tmp_path)
    assert validate_openheart_package_dir(package_dir) == []


def test_validate_openheart_package_dir_reports_tensor_name_mismatch(tmp_path: Path) -> None:
    package_dir = _write_minimal_valid_package(tmp_path)
    case_dir = package_dir / "cases" / "case_000001"
    np.savez_compressed(
        case_dir / "reference_outputs.npz",
    ...
    errors = validate_openheart_package_dir(package_dir)
